...
var ExecSync = module.exports = exports = function (args, callback) {
	var spawnSync = ChildProcess.spawnSync;
	if (!IS_WIN) {
		if (typeof args === 'string') {
			args = args.split(/\s+/);
		}
		const child = spawnSync('ps', args);
		var stdout = child.stdout.toString('utf8');
		var stderr = child.stderr.toString('utf8');
...
exports.lookup = function (query, callback) {
  ...
  return ExecSync(exeArgs, function (err, output) {
    if (err) {
      return callback(err);
    }
    else {
...
function formatOutput(data) {
  ...
  data.forEach(function (d) {
    ...
    // Naudit custom
    var etime = d['ELAPSED'] || undefined;
    var user = d['USER'] || undefined;
